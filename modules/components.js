import { app, temp } from '/modules/global.js';
import { updateClock, userPageEvents, newUserPageEvents, globalEvents } from '/modules/events.js';
import { getUser, appData, getWeather, getQuote, featureDetection } from '/modules/CRUD.js';
...
function userProjectsPage() {
    ...
    let main = app.querySelector('main');
    let html = `
    <ul class="projects__overview" aria-label="Project's list">
                <li class="list__header" aria-label="Project's list header">
                <div class="project__element"><i aria-hidden="true" class="i ri:at-line"></i><span>Project's Name</span></div>
                <div class="project__element">Duration</div>
...
function userProjectPage(id) {
    user = getUser();
    let project = user.getItemById(id, 'projects');
    let main = app.querySelector('main');
    let html = `
    <section class="overview__project transition-shadow">${project.name}</section>
    <section class="overview__tasks tasksPage" aria-label="Tasks Overview" id="overview-tasks">
        <section class="overview__tasks-todo  transition-shadow">
        ${listItems({ array: project.getTasksByStatus(user, 'todo'), type: "tasks" })}
        </section>
        <section class="overview__tasks-inprogress  transition-shadow">
        ${listItems({ array: project.getTasksByStatus(user, 'progress'), type: "tasks" })}
        </section>
        <section class="overview__tasks-done transition-shadow">
        ${listItems({ array: project.getTasksByStatus(user, 'done'), type: "tasks" })}
        </section>
    </section>
    `
    main.innerHTML = html;
...
export function listItems(values) {
    ...
    `);
        ...
        <article class=" transition-shadow card show-hover" aria-label="Note for: ${item.title}">
           ...
           ${item.project ? `<button type="button" data-page="userProjectPage" data-id="${item.project}" class="notes__project card__label">${user.getItemById(item.project, 'projects').name}</button>` : ''}
    ...
    `);
        ...
        array.map(item => html += `
                ${before}
                <li tabindex=0 data-type="projects" data-id="${item.id}" data-page="userProjectPage" aria-label="${item.name}" class="button-type">
                <div class="project__element"><i aria-hidden="true" class="i ri:at-line"></i><span>${item.name}</span></div>
                <div class="project__element">${formattedTime(item.getProjectDuration(user))}</div>
                <div class="flex-group">
                <span class="label prod__todo">${item.getTasksByStatus(user, 'todo').length} to do </span>
                <span class="label prod__inprogress">${item.getTasksByStatus(user, 'progress').length} on going</span>
                <span class="label prod__done">${item.getTasksByStatus(user, 'done').length} done</span></div>
                </li>
                ${after}
        `);
        return html
        ...
        <div class="card-group">
            ...
            ${item.project ? `<button data-id="${item.project}" data-type="projects" data-page="userProjectPage" data-id="${item.project}" class="card__label">@${user.getItemById(item.project, 'projects').name}</button>` : ''}
        </div>
    ...
    `);
        ...
        <div class="card-group">
            <h1 class="card__title">${item.task}</h1>
            ${item.project ? `<button data-id="${item.project}" data-page="userProjectPage" data-id="${item.project}" data-type="projects" class="card__label">@${user.getItemById(item.project, 'projects').name}</button>` : ''}
        </div>
        <div class="card-group card__btns">
            <button type="button" data-id="${item.id}" data-type="tasks" class="modal-edit-btn show-element">
                <i class="i ic:outline-edit" aria-hidden="true"></i>
